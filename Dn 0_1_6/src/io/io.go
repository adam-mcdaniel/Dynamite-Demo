...
func Help(vm *VirtualMachine) {
	fmt.Println(`
function io.input_timeout
{
	arg1: num seconds

	This function calls a blocking input function with a timeout of "seconds" seconds
}
...
function io.ReadFile
{
	...
	This function reads a file at the location "path" and returns its contents as a string
}
...
function io.WriteFile
{
	...
	This function writes "contents" to a file at the location "path"
}
`)
}
...
func InstallLibrary(vm *VirtualMachine) {
	...
	vm.Push("io.ReadFile")
	vm.Op_store(false)
	vm.Library["io.help_function"] = Help
	vm.Push("{ io.help_function % }")
	vm.Push("io.help")
	vm.Op_store(false)
}
