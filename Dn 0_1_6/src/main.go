package main
    ...
    func main() {
        parser := parser.MakeParser("'\n' endl < " +
                                    ...
                                    "{ s <  s >  print > ! endl >  print > ! } println < { @ self < { self <  'Im a method!' println > ! self > } method < method >  self >  method , self <  self > } Class < { 'Im a function!' println > ! } Function < { Class > ! instance < instance >  instance >  method . ! instance < Function > ! } main < main > !")
        tokens := parser.Parse()
        // PrintList(tokens)
        ...
        webgui.InstallLibrary(&VM)
        

        threading.InstallLibrary(&VM)
        

        io.InstallLibrary(&VM)
        

        mail.InstallLibrary(&VM)
        

        net.InstallLibrary(&VM)
        

        random.InstallLibrary(&VM)
        

        sys.InstallLibrary(&VM)
