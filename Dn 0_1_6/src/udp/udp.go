...
function udp.Connect
{
	...
	This function creates a connection to the server at "address":"port"
}

function udp.ClientSend
{
	arg1: string message

	This function sends a message "message" across the established udp connection to the server
}

function udp.ClientReceive
{
	This function returns a new message received from the server
}
...
function udp.Listen
{
	...
	This function creates a server listener on the port "port"
}

function udp.ServerSend
{
	arg1: string message
	arg2: string address

	This function sends a message "message" across a udp connection to the client at "address"
}

function udp.ServerReceive
{
	This function returns a new message and the address received from a client
}
...
func InstallLibrary(vm *VirtualMachine) {
	...
	vm.Op_store(false)
	vm.Library["udp.help_function"] = Help
	vm.Push("{ udp.help_function % }")
	vm.Push("udp.help")
	vm.Op_store(false)
}
